...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Button asChild variant="outline" className="h-auto py-4 flex flex-col gap-2">
          <Link to="/match">
            ...
            <span className="text-sm font-medium">Encontrar colegas</span>
          </Link>
        </Button>
      </div>
        ...
        <CardContent>
          {matchLoading ? (
            ...
            <p className="text-sm text-muted-foreground">
              Nenhum match ainda. <Link to="/match" className="text-primary underline">Encontre colegas!</Link>
            </p>
          ) : (
      ...
      {/* Favorited Properties Carousel */}
      <div>
        <h2 className="text-lg font-semibold mb-3">Imóveis Favoritos</h2>
        {propLoading ? (
          <div className="flex gap-4">
            {[1, 2].map((i) => <Skeleton key={i} className="h-64 w-72 rounded-lg shrink-0" />)}
          </div>
        ) : favorited.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Nenhum favorito ainda. <Link to="/marketplace" className="text-primary underline">Explore o marketplace!</Link>
          </p>
        ) : (
