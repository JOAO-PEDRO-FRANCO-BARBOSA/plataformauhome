...
const amenityIcons: Record<string, React.ElementType> = {
  ...
  'Mobiliado': Sofa,
  'Quintal': Trees,
  'Quintal grande': Trees,
  'Cozinha compartilhada': CookingPot,
  'Cozinha equipada': CookingPot,
  'Área de estudo': BookOpen,
  'Lavanderia': CookingPot,
  'Varanda': Trees,
  'Área gourmet': CookingPot,
  'Perto do campus': MapPin,
};
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                  {images.map((_, i) => (
                    <button
                      ...
                      className={`w-2.5 h-2.5 rounded-full transition-colors ${i === imgIndex ? 'bg-white' : 'bg-white/50'}`}
                    />
                  ))}
                </div>
              </>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {property.acceptsPet && (
                <Badge className="bg-amber-500 text-white flex items-center gap-1">
                  <PawPrint className="w-3 h-3" /> Aceita Pet
                </Badge>
              )}
            </div>
        ...
        {/* Right: Price Card (desktop sticky / mobile fixed bottom) */}
